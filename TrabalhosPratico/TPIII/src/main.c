...
{int i,nelements;
 if(root1->left !=NULL){nelements++;
    int *leafs=(int*)malloc( nelements*sizeof(int)) ;  

    for (size_t i = 0; i < NULL; i++)
 {leafs=realloc(leafs,nelements*sizeof(int));
   leafs++;
   
 }
 }
}*/
void getLeaf(struct TreeNode * Root, int * out, int * idx)
{
    if(Root == NULL) return;
    ...
    {
        getLeaf(Root->left, out, idx);
        getLeaf(Root->right, out, idx);
    }
}
bool leafSimilar(struct TreeNode *root1, struct TreeNode *root2)
...
int *leafs1=(int*)malloc( nelements*sizeof(int)) ;  
    ...
    if(memcmp(leafs,leafs1,200) == 0) {return true;
...
int POP(Treebin *Root, int value)
{
    if (Root == NULL)
        return 0;
    struct TreeNode *new;
    new = (struct Node *)malloc(sizeof(struct TreeNode));
    if (new == NULL)
        return 0;
    new->val = value;
    new->right = NULL;
...
Treebin *Root2 = create_TreeBin();
    ...
    bool value= leafSimilar(Root2, Root3);
